...
run_deseq2 <- function(count_mat, baseline_condition, contrast_condition, name = "") {
  ...
  res_df$padj <- ifelse(res_df$padj < 1e-14, 1e-14, res_df$padj)
...
# Select top 10 by significance (lowest padj) per method
top10_annotated <- combined_annot %>%
  filter(!is.na(padj)) %>%
  arrange(padj) %>%
  group_by(method) %>%
  slice_head(n = 10) %>%
...
volcano_combined <- ggplot(combined_res, aes(x = logFC, y = -log10(padj))) +
  ...
  geom_text_repel(
    data = top10_annotated,
    aes(label = annotation),
    color = "gray20",
    size = 2.5,
    max.overlaps = Inf,          # show all selected labels
    box.padding = 0.4,           # increase space around label slightly
    point.padding = 0.5,         # a bit more room around point
    force = 2,                 # stronger repulsion between labels
    force_pull = 0.5,           # gentle pull back to point
    segment.size = 0.3,
    segment.color = "gray20",
    min.segment.length = 0,
    direction = "both",          # allow full adjustment
    seed = 42
  )
