...
# Select top 7 by significance (lowest padj) per method
top7_annotated <- combined_annot %>%
  filter(!is.na(padj)) %>%
  arrange(padj) %>%
  group_by(method) %>%
  slice_head(n = 7) %>%
...
2.  Define promoter regions
    ...
    The promoter region was defined as ±5 kb around the TSS to keep it consistent with the paper.
...
3.  Prepare peaks
    ...
    To obtain peaks for each condition, we performed the naive overlap strategy (<https://www.encodeproject.org/data-standards/atac-seq/atac-encode4/>) on the replicates in each condition. First, for each condition, the aligned files from all replicates were combined and peak calling was performed to get the pooled peaks. To get the set of peaks for each condition, only pooled peaks that overlap (≥50% overlap in peak length) with peaks in the replicates were kept. This ensure reproducibility in the downstream motif enrichment analysis.

    Next, we prepare the set of peaks that are common and unique between two conditions as defined here:

    -   Group 1 (Uninjured and injured Myofibers):

        1.  Peaks common to uninjured and injured Myofibers

        2.  Peaks unique to uninjured Myofibers

        3.  Peaks unique to injured Myofibers

    -   Group 2 (WT and MDX Myofibers):

        1.  Peaks common to WT and MDX Myofibers

        2.  Peaks unique to WT Myofibers

        3.  Peaks unique to MDX Myofibers
    ...
    Then, we filtered for peaks that overlaps with the promoter regions.
...
4.  Run motif enrichment analysis
    ...
    We performed motif enrichment using MEME’s AME tool (Robert McLeay and Timothy L. Bailey, "Motif Enrichment Analysis: A unified framework and method evaluation", BMC Bioinformatics, 11:165, 2010, <doi:10.1186/1471-2105-11-165>) with its defaults (i.e. `avg` scoring, one‑tailed Fisher’s test, and a shuffled background), rather than the HOMER settings used in the original paper.
    ...
    ```{r, eval=FALSE}
    motif_analysis_by_group(peaks$ui_final, peaks$i_final, "ui_i_motifs.rds", "ui_motifs.rds", "i_motifs.rds")
    motif_analysis_by_group(peaks$mdx_final, peaks$wt_final, "mdx_wt_motifs.rds", "mdx_motifs.rds", "wt_motifs.rds")
    ```
...
5.  Visualize top 5 enriched motifs for each set of peaks
    ...
    motif_visualization <- function(file_name, overlap_val=0.99, plot_title){
    ...
    motif_visualization(paste0(motif_dir, "ui_i_motifs.rds"), plot_title="Top 5 motifs enriched in peaks common to Uninjured and Injured\nMyofibers overlapping the promoters")
    motif_visualization(paste0(motif_dir, "ui_motifs.rds"), plot_title="Top 5 motifs enriched in peaks unique to Uninjured Myofibers\noverlapping the promoters")
    motif_visualization(paste0(motif_dir, "i_motifs.rds"), plot_title="Top 5 motifs enriched in peaks unique to Injured Myofibers\noverlapping the promoters")

    motif_visualization(paste0(motif_dir, "mdx_wt_motifs.rds"), plot_title="Top 5 motifs enriched in peaks common to MDX and WT Myofibers\noverlapping the promoters")
    motif_visualization(paste0(motif_dir, "wt_motifs.rds"), plot_title="Top 5 motifs enriched in peaks unique to WT Myofibers\noverlapping the promoters")
    motif_visualization(paste0(motif_dir, "mdx_motifs.rds"), plot_title="Top 5 motifs enriched in peaks unique to MDX Myofibers\noverlapping the promoters")
    ```

    Our results differed from the original paper's top 5 enriched motifs, likely due to methodological differences between MEME (AME) and HOMER. Some differences that could explain the discrepancies in the results are listed below.

+--------------------------------------------------+--------------------------------------------------------------------------------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Feature                                          | MEME (AME)                                                                                                                                       | HOMER (findMotifsGenome.pl)                                                                                                                                    | Potential impact                                                                                                                                                          |
+==================================================+==================================================================================================================================================+================================================================================================================================================================+===========================================================================================================================================================================+
...
1.  We used a naive overlap approach to obtain the set of peaks for each condition, giving us the strongest and most reproducible peak regions. However, due to this, true but variable binding sites that are not captured in all replicates may be lost. To obtain a more confident and reproducible set of peaks, Irreproducible Discovery Rate (IDR) could be used for future work.
2.  By default MEME (AME), does not take into account the GC content when generating the background model. However, regions near the TSS tend to have high GC content and is a significant source of sequence content bias in mammalian genomes. Hence, the top enriched motifs identified using MEME (AME) may be bias. In future work, a custom background model which take into account the GC content could be created and and defined when running meme::ame.
