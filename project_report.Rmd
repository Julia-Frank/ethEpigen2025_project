...
*Fig. 1: Heatmap showing the ATAC-seq signal around the transcription start sites (TSS) of uninjured, injured, WT, and MDX myofibers.*
...
*Fig. 2: Heatmaps of Pearson correlation coefficients based on normalized ATAC-seq counts, illustrating the correlation between replicates in the Uninjured vs. Injured comparison (left) and the WT vs. MDX comparison (right).*
...
*Fig. 3: Principal component analysis (PCA) plot showing samples along the first two principal components for Uninjured vs. Injured (left) and MDX vs. WT (right).*
...
*Fig. 4: Hierarchical clustering dendrograms of normalized ATAC-seq counts showing sample relationships for Uninjured vs. Injured (left) and MDX vs. WT (right) conditions.*
...
```{r differential accessibility analysis, warning=FALSE}
...
*Fig. 5: Volcano plots of differential chromatin accessibility between conditions for Uninjured vs. Injured (left) and WT vs. MDX (right). Points represent peaks colored by significance, with top significant peaks labeled by their nearest gene and distance to that gene.*
...
: Comparison of MEME (AME) and HOMER (findMotifsGenome.pl) features and their potential impact.
...
2.  By default, MEME’s AME tool does not correct for GC‐content bias when generating its background model. Because promoter regions (±5 kb of TSS) are inherently GC‐rich, this omission can inflate enrichment statistics for GC‐rich motifs [@heinzSimpleCombinationsLineageDetermining2010]. To address this, future analyses should employ a custom, GC‐matched background when running AME.
