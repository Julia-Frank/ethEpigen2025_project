...
Differential accessibility analysis was performed using `DESeq2`, where counts from each experiment were compared by setting one condition as the baseline (Uninjured or WT) and testing for differences in accessibility against the contrasting condition (Injured or MDX).
...
Gene set enrichment was carried out with `great`[@guRGREATBioconductorPackage2023] using MSigDB Hallmark gene sets on significant differentially accessible regions (DARs). By working directly with genomic intervals, rather than a ranked gene list as in the original paper, we instead ask: *Which hallmark pathways are overrepresented among regions that gain or lose accessibility between conditions?* To answer this, we ran `great` separately on up‑ and down‑regulated DARs (injured compared to uninjured myofibers and MDX compared to WT myofibers), using the union of all consensus peaks as our background.
...
As expected, given our region-centric enrichment approach rather than the paper's ranked genes method, the order of enriched pathways differs. Nonetheless, we still observe key inflammatory and injury related pathways (e.g. IL6–JAK–STAT3 [@zhangInterleukin6SignalTransducer2013], Complement activation [@yangSkeletalMuscleRegeneration2018]) among the top 10 upregulated pathways in injured vs uninjured myofibers, consistent with the original findings (Fig. 7).

Similarly, as seen in Fig. 8, we observed pathways, such as IL6–JAK–STAT3 [@zhangInterleukin6SignalTransducer2013] and wnt/β-Catenin [@liuActivationWntVCatenin2016], which have been reported to be upregulated in Duchenne’s muscular dystrophy (DMD).
...
2.  By default, MEME’s AME tool does not correct for GC‐content bias when generating its background model. Since promoter regions (±5 kb of TSS) are inherently GC‐rich, this omission can inflate enrichment statistics for GC‐rich motifs [@heinzSimpleCombinationsLineageDetermining2010]. Indeed, our results identified several GC-rich motifs (Fig. 7, Supplementary Fig. 1), suggesting a potential bias. To address this, future analyses should employ a GC-matched background when running AME.

# Conclusion

In conclusion, we successfully reproduced and critically evaluated smfATAC‑seq profiling of single myofibers under regenerative (uninjured vs. injured) and dystrophic (WT vs. MDX) conditions, implementing a complete preprocessing and analysis pipeline (including trimming, alignment, peak calling, consensus peak generation, normalization, differential accessibility testing, region‑centric gene set enrichment (GREAT), and MEME‑based motif enrichment), while carefully addressing batch effects by analyzing experiments separately. We confirmed high data quality via TSS enrichment and strong replicate consistency, observed larger accessibility shifts during regeneration but a greater number of subtler changes in the MDX model, and identified key inflammatory and injury‑related pathways (e.g., IL6–JAK–STAT3, complement activation, Wnt/β‑catenin) consistent with previous findings and the original paper. Although our motif enrichment analysis diverged from the original paper, likely due to methodological differences between AME and HOMER and the lack of GC‑matched backgrounds, our results highlights both the robustness of smfATAC‑seq for interrogating chromatin dynamics in single myofibers and the importance of analytic choices in shaping biological interpretation.
