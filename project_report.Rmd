...
Gene set enrichment was carried out with `GREAT`[@guRGREATBioconductorPackage2023] using MSigDB Hallmark gene sets on significant differentially accessible regions (DARs). Instead of using a ranked gene list as in the original paper, we worked directly with genomic intervals to answer the question: *Which hallmark pathways are overrepresented among regions that gain or lose accessibility between conditions?* To adress this, we ran `GREAT` separately on up‑ and down‑regulated DARs (injured compared to uninjured myofibers and MDX compared to WT myofibers), using the union of all consensus peaks as our background.
