...
-   Redefine question answered (different from paper)
    -   Explain rationale for doing so
    -   how this impacts the analysis
    -   Difference: gene sets
-   Unknown chromosomes
    -   Explain rationale on why we drop the unknown chromosomes
    -   Explain the possible impact of dropping unknown chromosomes
...
dars_gr <- function(dars, consensus_peak){
  ...
  # Count # of non-standard chromosomes
  standard_chroms <- paste0("chr", c(1:19, "X", "Y", "M"))
  non_std <- sum(!(as.character(seqnames(dars)) %in% standard_chroms))
  print(paste0("Number of peaks with non-standard chromosomes: ", non_std))
  print(paste0("Number of peaks: ", length(dars)))
  perc <- round((non_std / length(dars) * 100), 2)
  print(paste0("Percentage of non-standard chromosomes in consensus peaks of UI and I: ", perc, " %"))
...
run_great <- function(dars_gr, consensus_peak){
  ...
  hm <- getEnrichmentTables(res, min_region_hits = 0)
...
plot_enrichment <- function(hm_up, hm_down){
  p1 <- ggplot(head(hm_up,10), aes(fold_enrichment, reorder(id, p_adjust), 
                        size=observed_region_hits, color=-log10(p_adjust))) + 
    geom_point() + scale_color_viridis_c() +
    ggtitle("(a) Upregulated pathways") +
    plot_formatting
    
  p2 <- ggplot(head(hm_down,10), aes(fold_enrichment, reorder(id, p_adjust), 
                        size=observed_region_hits, color=-log10(p_adjust))) + 
    geom_point() + scale_color_viridis_c() +
...
3.  Prepare peaks
    ...
    *Table 1: Peak categories across myofiber conditions.*
    ...
    We merged all peaks across both conditions and defined common peaks as those that appear in both conditions, while unique peaks were defined as those not overlapping any common peak.
    ...
    obtain_peaks <- function(cond1, cond2){
      # Find common peaks
      common_peaks <- GRangesList(list(cond1, cond2))
      common_peaks <- GenomicRanges::reduce(unlist(common_peaks), with.revmap=TRUE)
      common_peaks <- common_peaks[which(lengths(common_peaks$revmap)>1)]
      
      # Unique peaks for each condition
      cond1_peaks <- cond1[!overlapsAny(cond1, common_peaks)]
      cond2_peaks <- cond2[!overlapsAny(cond2, common_peaks)]
